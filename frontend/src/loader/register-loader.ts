import { Loader, ModuleNamespace } from './loader-polyfill';
import { resolveIfNotPlain } from './resolve';
import { addToError, global, createSymbol, baseURI } from './common';
...
const REGISTER_INTERNAL = createSymbol('register-internal');

class RegisterLoader extends Loader{
  ...
  register(key, deps, declare) {
    ...
    // everything else registers into the register cache
      ...
      const load = state.records[key] || createLoadRecord(state, key, undefined);
   ...
   */
  registerDynamic(key, deps, executingRequire, execute) {
    const state = this[REGISTER_INTERNAL];
...
const INSTANTIATE = RegisterLoader['instantiate'] = createSymbol('instantiate');

// default normalize is the WhatWG style normalizer
RegisterLoader.prototype[RegisterLoader['resolve'] = Loader['resolve']] = (key, parentKey) => resolveIfNotPlain(key, parentKey || baseURI);

RegisterLoader.prototype[INSTANTIATE] = (key, processAnonRegister) => {};
...
RegisterLoader.prototype[Loader['resolveInstantiate']] = function (key, parentKey) {
  const loader = this;
  const state = this[REGISTER_INTERNAL];
  const registry = loader.registry[loader.registry._registry];
  ...
  .then(instantiated => {
    ...
    .then(() => ensureEvaluate(loader, instantiated, instantiated.linkRecord, registry, state, undefined))
    .catch(err => {
...
function resolveInstantiate (loader, key, parentKey, registry, state) {
  ...
  let module = registry[key];
  if (module)
    return Promise.resolve(module);

  let load = state.records[key];
...
function createProcessAnonRegister (loader, load, state) {
  return () => {
    const lastRegister = state.lastRegister;
...
function instantiate (loader, load, link, registry, state) {
  ...
  (load.registration ? Promise.resolve() : Promise.resolve().then(() => {
    state.lastRegister = undefined;
    return loader[INSTANTIATE](load.key, loader[INSTANTIATE].length > 1 && createProcessAnonRegister(loader, load, state));
  }))
  .then((instantiation: any) => {
    ...
    const registration = load.registration;
...
function resolveInstantiateDep (loader, key, parentKey, registry, state, traceDepMap) {
  ...
  .then(resolvedKey => {
    ...
    // normalization shortpaths for already-normalized key
    let load = state.records[resolvedKey];
    const module = registry[resolvedKey];
...
function registerDeclarative (loader, load, link, declare) {
  let moduleObj = link.moduleObj;
  const importerSetters = load.importerSetters;

  let locked = false;

  // closure especially not based on link to allow link record disposal
  const declared = declare.call(global, (name, value) => {
...
function instantiateDeps (loader, load, link, registry, state, seen) {
  ...
  .then(() => {
    const depsInstantiatePromises = Array(link.dependencies.length);
  ...
  .then(dependencyInstantiations => {
    link.dependencyInstantiations = dependencyInstantiations;

    // run setters to set up bindings to instantiated dependencies
    if (link.setters) {
      for (let i = 0; i < dependencyInstantiations.length; i++) {
        const setter = link.setters[i];
        if (setter) {
          const instantiation = dependencyInstantiations[i];
  ...
  .then(() => {
    ...
    const deepDepsInstantiatePromises = [];

    for (let i = 0; i < link.dependencies.length; i++) {
      const depLoad = link.dependencyInstantiations[i];
      const depLink = depLoad.linkRecord;

      if (!depLink || depLink.linked)
        continue;

      if (seen.includes(depLoad))
        continue;
      seen.push(depLoad);
...
function clearLoadErrors (loader, load) {
  const state = loader[REGISTER_INTERNAL];
  ...
  if (link.dependencyInstantiations)
    link.dependencyInstantiations.forEach((depLoad, index) => {
      ...
      if (depLoad.linkRecord) {
        ...
        if (link.setters && depLoad.importerSetters) {
          const setterIndex = depLoad.importerSetters.indexOf(link.setters[index]);
...
function ensureEvaluate (loader, load, link, registry, state, seen) {
  ...
  if (seen && seen.includes(load))
    return load.linkRecord.moduleObj;

  // for ES loads we always run ensureEvaluate on top-level, so empty seen is passed regardless
  // for dynamic loads, we pass seen if also dynamic
  const err = doEvaluate(loader, load, link, registry, state, link.setters ? [] : seen || []);
  if (err) {
  ...
  return name => {
    for (let i = 0; i < dependencies.length; i++) {
      if (dependencies[i] === name) {
        const depLoad = dependencyInstantiations[i];
        let module;
...
function doEvaluate (loader, load, link, registry, state, seen) {
  ...
  if (link.setters) {
    ...
    for (var i = 0; i < link.dependencies.length; i++) {
      ...
      if (depLink && !seen.includes(depLoad)) {
        if (depLink.error)
          err = depLink.error;
        else
  ...
  if (link.execute) {
    ...
    // "this" is "exports" in CJS
      ...
      const module: any = { id: load.key };
      const moduleObj = link.moduleObj;
      ...
      const require = makeDynamicRequire(loader, load.key, link.dependencies, link.dependencyInstantiations, registry, state, seen);
      ...
      if (moduleObj.default && moduleObj.default.__esModule) {
        ...
        for (const p in moduleObj.default) {
...
function dynamicExecute (execute, require, exports, module) {
  try {
    const output = execute.call(global, require, exports, module);
    if (output !== undefined)
